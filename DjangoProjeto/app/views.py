...
def index(request):
    ...
    context = get_contexto_logado(request, {'produtos_api': produtos_api})
        ...
        if frmLogin.is_valid():
            ...
            try:
                ...
                if check_password(_senha, userLogin.senha):
                    ...
                    request.session['is_admin'] = (_email == 'otho.01@hotmail.com')
...
def exibirUsuarios(request):
    ...
    if email_logado != 'otho.01@hotmail.com':
...
def checkout(request, produto_id):
    ...
    if request.method == 'POST':
        ...
        if produto.estoque >= quantidade:
            ...
            produto.save()
            return redirect('checkout_sucesso', produto_id=produto.id)
        else:
            context = get_contexto_logado(request, {
                'produto': produto,
                'usuario': usuario,
...
def checkout_sucesso(request, produto_id):
    produto = get_object_or_404(Produto, pk=produto_id)
    contexto = get_contexto_logado(request, {'produto': produto})
    return render(request, 'produtos/checkout_sucesso.html', contexto)
